...
| advsim-n8n | n8nio/n8n:latest (arm64) | 172.20.0.31 | 5679->5678 | Credentials set in .env |
...
| `/api/` | `172.20.0.31:5678/webhook/` | n8n webhook endpoints |
...
| n8n-data | advsim-n8n | n8n workflows + credentials |
...
|
+-- nginx/
|   +-- conf/default.conf           <- All proxy routes and WebSocket config
...
When the `labops-net` Docker network does not exist, the Makefile creates `advsim-net` (bridge, subnet `172.20.0.0/24`) and includes `docker-compose.guacamole.yml` to deploy its own Guacamole stack. All services are self-contained.
...
2. Does NOT include `docker-compose.guacamole.yml` -- Guacamole is provided by LabOps
3. All services share the same `172.20.0.0/24` subnet, so nginx proxy routes to Guacamole at `172.20.0.81` work regardless of which stack deployed it
